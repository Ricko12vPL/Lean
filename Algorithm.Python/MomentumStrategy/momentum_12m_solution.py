"""
Momentum Strategy 12M - Solution with CustomData reading LEAN format
======================================================================
Cross-Sectional Momentum Strategy with 12-month (252 days) lookback
...
class Momentum12MSolutionAlgorithm(QCAlgorithm):
    """Momentum Strategy with 12-month (252 days) lookback - solution version"""
    
    def initialize(self):
        """Initialize with 12M parameters"""
        # Backtest period
        ...
        # Add SPY as benchmark - use string directly to ensure data is loaded
        try:
            self.set_benchmark("SPY")  # Set SPY as benchmark for Alpha/Beta calculations
            # Also add SPY as equity to ensure data is available
            self.add_equity("SPY", Resolution.DAILY)
        except Exception as e:
            self.log(f"Warning: Could not set SPY benchmark: {e}")
        
        self.log("=" * 60)
        self.log("MOMENTUM 12M STRATEGY INITIALIZED (Solution Version)")
        self.log(f"Lookback: {self.lookback_days} days (12 months)")
